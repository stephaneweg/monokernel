...
sub FATFS_FILE.WRITE_CLUSTER()
    ...
    this.FS->DISK->WRITE((this.FS->Absolute_sector(this.CURRENT_CLUSTER)-1),this.FS->sector_per_cluster,this.CLUSTER)
    this.FS->DISK->WRITE((this.FS->Absolute_sector(this.CURRENT_CLUSTER)-1),this.FS->sector_per_cluster,this.CLUSTER)
...
sub FATFS_FILE.CLOSE()
    ...
    if (DIRTY=1) then
        ...
        if (entryNum<>0) then
            ...
            'write the fat
            this.FS->DISK->WRITE(FS->FIRST_FAT_SECTOR,this.FS->fat_sectors,this.FS->FAT_TABLE)
            ...
            'write the current cluster
            this.FS->DISK->WRITE((this.FS->Absolute_sector(CURRENT_CLUSTER)-1),this.FS->sector_per_cluster,CLUSTER)
            this.FS->DISK->WRITE((this.FS->Absolute_sector(CURRENT_CLUSTER)-1),this.FS->sector_per_cluster,CLUSTER)
            ...
            CONSOLE_WRITE_LINE(@"CANNOT FIND FREE ENTRY")
        end if
    end if
    
    KMM_FREEPAGEMULTIPLES(CLUSTER,CLUSTER_PAGES)
end sub