...
sub FAT_MOUNT(dev as unsigned byte ptr,path as unsigned byte ptr)
    CONSOLE_WRITE(@"MOUNT "):CONSOLE_WRITE(dev):CONSOLE_WRITE(@" AS FATFS :")
    var disk = BLOC_DEV_FIND(dev)
    if (disk<>0) then
        
        dim myboot as FAT_BOOTSECTOR ptr        
        dim MyBoot16 as BS16 ptr
        dim MyBoot32 as BS32 ptr
        myboot= KMM_ALLOCPAGE()
        ...
        if (disk->READ(0,1,cptr(unsigned byte ptr,myboot))=0) then
            CONSOLE_WRITE_LINE(@"UNABLE TO READ BLOCK DEVICE")
            return
        end if
        if (myboot->bps=0) or (myboot->spc=0) then
            KMM_FREEPAGE(myboot)
            CONSOLE_SET_FOREGROUND(12)
            CONSOLE_WRITE_LINE(@"BPS or SPC values is invalid")
        ...
        if (seccount=0) then
            KMM_FREEPAGE(myboot)
            CONSOLE_SET_FOREGROUND(12)
            CONSOLE_WRITE_LINE(@"INVALID SECTOR COUNT")
            CONSOLE_SET_FOREGROUND(7)
        end if
            
            
        
        dim retval as FATFS_DESCRIPTOR ptr = KERNEL_ALLOC(sizeof(FATFS_DESCRIPTOR))
        dim cpt as integer
        ...
        if (sign <> &h29 and sign <> &h0) then
            ...
            KERNEL_FREE(retval)
            CONSOLE_SET_FOREGROUND(12)
            CONSOLE_WRITE_LINE(@" Signature invalid")
            CONSOLE_SET_FOREGROUND(7)
            return
        end if
        ...
        retval->FAT_DirectoryBuffer = KMM_ALLOCPAGEMULTIPLES(65536 shr 12)
        ...
        retval->fat_table = KMM_ALLOCPAGEMULTIPLES(ftable_pages)
        retval->fat_table_pcount = ftable_pages
        
        retval->disk = disk
        retval->disk->READ(retval->first_fat_sector,retval->fat_sectors,retval->fat_table)
        ...
        CONSOLE_WRITE_LINE(@"UNABLE TO OPEN BLOCK DEVICE"):CONSOLE_WRITE_LINE(dev)
    end if
end sub     
...
sub FATFS_DESCRIPTOR.READ_ROOT(dst as FAT_ENTRY ptr)
    ...
    if (this.Fat_Type<>32) then
        this.DISK->READ((this.RESERVED_SECTORS+this.FAT_SECTORS),this.ROOT_DIR_SECTORS,cptr(unsigned byte ptr,dst))
	else
...
sub FATFS_DESCRIPTOR.WRITE_ROOT(src as FAT_ENTRY ptr)
    if (this.Fat_Type<>32) then
        this.DISK->WRITE((this.RESERVED_SECTORS+this.FAT_SECTORS),this.ROOT_DIR_SECTORS,cptr(unsigned byte ptr,src))
	else
...
sub FATFS_DESCRIPTOR.READ_CHAIN(clusternum as unsigned integer, dst as unsigned byte ptr)
    ...
    while(nxt<this.fat_limit)
        this.DISK->READ((this.Absolute_sector(nxt)-1),this.sector_per_cluster,buf)
...
sub FATFS_DESCRIPTOR.WRITE_CHAIN(clusternum as unsigned integer, src as unsigned byte ptr)
    ...
    cpt = 0
    while(nxt<this.fat_limit)
        this.DISK->WRITE((this.Absolute_sector(nxt)-1),this.sector_per_cluster,buf)
        nxt	=find_fatentry(nxt)
        buf +=(this.sector_per_cluster*this.bytes_per_sector)
        cpt+=1
    wend
...
function FATFS_DESCRIPTOR.READ_NEXT_CLUSTER(clusternum as unsigned integer,dst as unsigned byte ptr) as unsigned integer
    if (clusternum<this.FAT_LIMIT) then
        ...
        this.DISK->READ((this.Absolute_sector(clusternum)-1),this.sector_per_cluster,dst)
...
function FATFS_OPENREAD(fat as FATFS_DESCRIPTOR ptr,path as unsigned byte ptr) as unsigned integer
    ...
    if (clusternum<>0) then
        ...
        descr = KERNEL_ALLOC(sizeof(FATFS_FILE))
        ...
        descr->CLUSTER              = KMM_ALLOCPAGEMULTIPLES(descr->CLUSTER_PAGES)
    end if
    KERNEL_FREE(tmpPath)
    return cuint(descr)
end function
...
function FATFS_OPENWRITE(fat as FATFS_DESCRIPTOR ptr,path as unsigned byte ptr,appendMode as unsigned integer) as unsigned integer
    ...
    if (clusternumParentDir<>0) or (parentPath=0)  then
        ...
        CONSOLE_WRITE(@"DIRECTORY NOT FOUND : "):Console_Write_line(parentPATH)
    end if
    if (parentPath<>0) then KERNEL_FREE(parentPATH)
    KERNEL_FREE(tmpPath)
    return cuint(descr)
end function
...
function FATFS_READ(handle as unsigned integer,descr as any ptr,count as unsigned integer,dest as  unsigned byte ptr) as unsigned integer
    if (count<=0) then return 0
    dim descriptor as FATFS_FILE ptr = cptr(FATFS_FILE ptr,descr)
    if (descriptor->FPOS>=descriptor->FSIZE) then return 0
...
function FATFS_UMOUNT(fat as FATFS_DESCRIPTOR ptr) as unsigned integer
    KMM_FREEPAGEMULTIPLES(fat->FAT_DIRECTORYBUFFER,65536 shr 12)
    KMM_FREEPAGEMULTIPLES(fat->FAT_TABLE,fat->FAT_TABLE_PCOUNT)
    fat->FAT_TABLE = 0
    fat->DISK = 0
