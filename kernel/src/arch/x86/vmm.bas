...
function KMM_ALLOCPAGEMULTIPLES(cpt as unsigned integer) as any ptr
    var virt = current_context->FIND_FREE_PAGES(cpt,VMM_START_ALLOC, PROCESS_ADDRESS)
    if (virt<>0) then
        var addr = cuint(virt)
        for i as unsigned integer = 0 to cpt-1
            var paddr = PMM_ALLOCPAGE()
            current_context->map_page(cptr(any ptr,addr),paddr, VMM_FLAGS_KERNEL_DATA)
            addr+=PAGE_SIZE
        next
    end if
    return cptr(any ptr,virt)
end function

sub KMM_FREEPAGEMULTIPLES(addr as any ptr,cpt as unsigned integer)
    var virt = cuint(addr)
    for i as unsigned integer = 0 to cpt -1
        KMM_FREEPAGE(cptr(any ptr,virt))
        virt+=PAGE_SIZE
    next i
end sub
...
sub VMM_INIT()
    CONSOLE_WRITE(@"Initializing Virtual Memory Management")
    paging_active = 0
...
function vmm_kernel_automap (p_start as any ptr, size as unsigned integer, flags as unsigned integer ) as any ptr
	return vmm_get_current_context()->automap(p_start, size, VMM_START_ALLOC, PROCESS_ADDRESS, flags)
end function
